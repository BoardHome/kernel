...
#define LINE_IN_NO   0
static int line_in_status = LINE_IN_NO;
struct rt5640_priv g_rt5640;
static int gstate = 4;
...
static void rt5640_hp_gpio_ctrl(struct rt5640_priv *rt5640, bool enable)
{
    ...
    if((LINE_IN_OKAY == line_in_status) && (false == enable) ) {
        return;
    }
...
static void rt5640_set_linein(struct rt5640_priv *rt5640)//struct snd_soc_component *code/)
{
    printk("%s enter\n",__func__);
...
static int rt5640_aux_adc_iio_read(struct rt5640_priv *rt5640)
{
	...
	if (ret < 0) {
		dev_err(rt5640->component->dev, "read aux_det channel() error: %d\n", ret);
		return ret;
	}
	return val;
}
			   ...
			   struct snd_ctl_elem_value *ucontrol)
{
	...
	if (rt5640->asrc_en) {
    	...
    	snd_soc_component_update_bits(component, RT5640_OUT_R3_MIXER,    //52 mute recmixr
                                1<<3 , 1<<3);
		regmap_write(rt5640->regmap, RT5640_PWR_MIXER, 0xf000);
		regmap_write(rt5640->regmap, RT5640_PWR_VOL, 0xf000);
	}
...
static ssize_t mic_state_show(struct device *dev, \
        struct device_attribute *attr, char *buf)
{
    return sprintf(buf, "%u\n", gstate);
}

static struct kobject *mic_state_kobj;
static DEVICE_ATTR(mic_state, (S_IWUSR|S_IRUSR|S_IWGRP|S_IRGRP), mic_state_show, mic_state_store);
	...
	case SND_SOC_BIAS_STANDBY:
		if (SND_SOC_BIAS_OFF == snd_soc_component_get_bias_level(component)) {
			...
			snd_soc_component_update_bits(component, RT5640_MICBIAS,
						0x0030, 0x0030);
			snd_soc_component_write(component, RT5640_PWR_ANLG2, 0x0800); //micbas1
		}
		break;
...
{
	snd_soc_card_jack_new(rt5640->component->card, "Headphone Jack", SND_JACK_HEADPHONE,
...
static int rt5640_hp_adc_iio_read(struct rt5640_priv *rt5640)
{
	...
	ret = iio_read_channel_raw(channel, &val);
	if (ret < 0) {
		dev_err(rt5640->component->dev, "read hp_det channel() error: %d\n", ret);
		return ret;
	}
	return val;
}
	...
	if (1/*!rt5640->in_suspend*/) {
		...
		if (result > RT5640_ADC_INVALID_ADVALUE && result < RT5640_ADC_EMPTY_ADVALUE){
			...
			if (result < rt5640->hp_det_adc_value + RT5640_ADC_DRIFT_ADVALUE &&
				...
				if(!rt5640->hp_insert){
					dev_dbg(rt5640->component->dev, "headphone insert, adc=%d\n", result);
 				...
 				if(rt5640->hp_insert){
					dev_dbg(rt5640->component->dev, "headphone not insert, adc=%d\n", result);
...
static void rt5640_hp_det_gpio_poll(struct work_struct *work)
{
	struct rt5640_priv *rt5640;
	int result = -1;

	rt5640 = container_of(work, struct rt5640_priv, hp_det_gpio_poll_work.work);
	...
	schedule_delayed_work(&rt5640->hp_det_gpio_poll_work, RT5640_ADC_SAMPLE_JIFFIES);
}
...
static int rt5640_probe(struct snd_soc_component *component)
{
	...
	if (rt5640->dmic_en)
		rt5640_dmic_enable(component, rt5640->dmic1_data_pin,
					  rt5640->dmic2_data_pin);
	...
	if (rt5640->chan) {
		...
		schedule_delayed_work(&rt5640->adc_poll_work,
				      1000);
	}else if (rt5640->hp_det_gpio){
		rt5640_jack_init(rt5640);
		INIT_DELAYED_WORK(&rt5640->hp_det_gpio_poll_work, rt5640_hp_det_gpio_poll);
			schedule_delayed_work(&rt5640->hp_det_gpio_poll_work,
						  1000);
	}
		...
		struct device_attribute *attr, const char *buf,size_t count)
{
	...
	ret = kstrtoul(buf, 10, &state);
	if (state == 1){
		rt5640_set_linein(&g_rt5640);
		line_in_status = LINE_IN_OKAY;
		gval = 1;
	} else {
		gpio_set_value(g_rt5640.hp_con_gpio,  0 );
		line_in_status = LINE_IN_NO;
		gval = 0;
	}
	c = state;
...
static int rt5640_parse_dt(struct rt5640_priv *rt5640, struct device *dev)
{
	...
	if (IS_ERR(chan)) {
		printk("rt5640 have no io-channels-aux defined\n");
		...
		if (!of_property_read_u32(np, "aux-det-adc-value", &adc_value)){
			...
			aux_chan = NULL;
			printk("rt5640 have no aux_det_adc_value defined\n");
	...
	else
	{
		...
		ret = devm_gpio_request(dev, gpio, "hp-det-gpio");
		if(ret < 0){
			gpio_free(gpio);
			dev_err(dev, "hp-det-gpio request ERROR\n");
		}else{
	...
	rt5640->in1_diff = of_property_read_bool(np,
					"realtek,in1-differential");
	rt5640->in2_diff = of_property_read_bool(np,
					"realtek,in2-differential");

	rt5640->ldo1_en = of_get_named_gpio(np,
	 ...
	 */
	if (!gpio_is_valid(rt5640->ldo1_en) && (rt5640->ldo1_en != -ENOENT))
		    ...
		    const struct i2c_device_id *id)
{
	...
	mic_state_kobj = kobject_create_and_add("mic_state", NULL);
	if (mic_state_kobj == NULL) {
		printk("create mic_state kobject fail \n");
		ret = -ENOMEM;
		goto err;
	}

	ret = sysfs_create_file(mic_state_kobj, &dev_attr_mic_state.attr);
	if (ret) {
		printk("mic_state firefly_sysfs_init: sysfs_create_group failed\n");
		goto err;
	}

	return devm_snd_soc_register_component(&i2c->dev, &soc_component_dev_rt5640,
				      rt5640_dai, ARRAY_SIZE(rt5640_dai));
err:

	kobject_del(mic_state_kobj);
	return devm_snd_soc_register_component(&i2c->dev, &soc_component_dev_rt5640,
				      rt5640_dai, ARRAY_SIZE(rt5640_dai));
}
